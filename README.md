...
LoRA updates are invariant to rotations of the latent rank space: `(B_i R_i)(R_i^T A_i) = B_i A_i`. Although the product is unchanged, the individual factors may live in different client-specific coordinate systems. FedRot-LoRA solves an orthogonal alignment problem for each client before averaging the factors.
